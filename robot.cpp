...
{std::cout<<"data z kobuki "<<std::endl; return 0; };
std::function<int(LaserMeasurement)> Robot::do_nothing_laser = [](LaserMeasurement data)
{std::cout<<"data z rplidar "<<std::endl; return 0; };
...
void Robot::robotprocess()
{
    ...
    {
        std::cout << "success with socket" << std::endl;
    }
    ...
    {
        std::cout << "success with sending to robot" << std::endl;
    }
#ifdef _WIN32
    Sleep(100);
#else
    ...
    {
        std::cout << "success with sending sound" << std::endl;
    }
    ...
    while (1)
    {
        ...
        if (returnval == 0)
        {
            ...
            std::async(
                std::launch::async, [this](TKobukiData sensdata)
                { robot_callback(sensdata); },
                sens);
        }
    }
...
void Robot::laserprocess()
{
    ...
    while (1)
    {
        ...
        // if (readyFuture.wait_for(std::chrono::seconds(0)) == std::future_status::ready)
        // {
        //     std::cout << "breaking this shit from laser process" << std::endl;
        //     break;
        ...
        std::async(
            std::launch::async, [this](LaserMeasurement sensdata)
...
{
    std::cout << "setting robot threads" << std::endl;
    if (wasRobotSet == 1)
    {
